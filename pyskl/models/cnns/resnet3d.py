...
class CBAMLayer(nn.Module):
    def __init__(self, channel, reduction=16, spatial_kernel=7):
        ...
        # Channel attention: compress H, W, D to 1
        ...
        # Spatial attention
        # Input to the spatial attention conv layer will be 2 channels (max and avg pooled along channels)
        # but these are still 3D tensors (H, W, D)
    ...
    def forward(self, x):
        ...
        # Spatial attention
        # Pooling across channels to get (H, W, D) tensors
        max_out, _ = torch.max(x, dim=1, keepdim=True)  # Max pool across channels
        avg_out = torch.mean(x, dim=1, keepdim=True)  # Average pool across channels

        # Concatenate along channel dimension (now we have 2 channels: max and avg)
        # and reshape to add a batch dimension (N, 2, H, W, D)
        concat_out = torch.cat([max_out, avg_out], dim=1)

        # Apply spatial attention convolution
        spatial_out = self.sigmoid(self.conv(concat_out))

        # Scale the input by the spatial attention map
        x = spatial_out * x
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.SE = SE_Block(self.planes * self.expansion)
        # self.CBAM = CBAMLayer(self.planes * self.expansion)
...
class ResNet3d(nn.Module):
    ...
    def _make_bottleneck_layer(self):
        self.basic_block1 = self.make_res_layer(
            self.basic_block,
            768,
            512,
            self.num_block,
    ...
    def forward(self, x):
        ...
        #                 ], 1)
        # x3 = F.interpolate(y_list[1], size=(x2_t, x2_h, x2_w), mode='trilinear', align_corners=True)
        # x2 = y_list[2] - x3
        x2 = torch.cat([y_list[2], \
                        F.interpolate(y_list[1], size=(x2_t, x2_h, x2_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x2 = self.se_block1(x2)
        x2 = self.basic_block1(x2)
        outs.append(x2)
        if len(outs) == 1:
            return outs[0]
