...
class Bottleneck3d_NoExpansion(nn.Module):
    """Bottleneck 3d block for ResNet3D.
...
class ResNet3d(nn.Module):
    ...
    no_expansion = [Bottleneck3d_NoExpansion]
    basic_block = [BasicBlock3d]
    se_block = [SE_Block]
    ...
    def __init__(self,
        ...
        # it is important for depth
        self.block, stage_blocks = self.arch_settings[depth]
        self.num_block = num_block
        self.block_NoExpansion = self.no_expansion[0]
        self.basic_block = self.basic_block[0]
        self.se_block = self.se_block[0]
        ...
        self._make_bottleneck_layer()
        self._make_seblock_layer()
        self.res_layers = []
    ...
    def _make_bottleneck_layer(self):
        self.bottleneck_layer1 = self.make_res_layer(
            self.block_NoExpansion,
            384,
            128,
            self.num_block,
            stride=(1, 1),
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer2 = self.make_res_layer(
            self.block_NoExpansion,
            640,
            128,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer3 = self.make_res_layer(
            self.block_NoExpansion,
            384,
            256,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.basic_block3 = self.make_res_layer(
            self.basic_block,
            384,
            256,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer4 = self.make_res_layer(
            self.block_NoExpansion,
            768,
            256,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer5 = self.make_res_layer(
            self.block_NoExpansion,
            640,
            512,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer6 = self.make_res_layer(
            self.block_NoExpansion,
            768,
            512,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.basic_block6 = self.make_res_layer(
            self.basic_block,
            768,
            512,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.bottleneck_layer7 = self.make_res_layer(
            self.block_NoExpansion,
            1024,
            512,
            self.num_block,
            ...
            inflate=0,
            inflate_style=self.inflate_style)
        self.basic_block7 = self.make_res_layer(
            self.basic_block,
            1024,
            512,
            self.num_block,
    ...
    def _make_seblock_layer(self):
        self.se_block1 = self.se_block(768)
        self.se_block2 = self.se_block(384)
        self.se_block3 = self.se_block(1024)
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            if i in self.out_indices:这里是判断是否是最后一层，如果是最好一层，就把最后一层的结果存到outs中。
        """
        x = self.conv1(x)
        x = self.maxpool(x)
        y_list = []
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            #     outs.append(x)
        # 第一个主流
        x2_h, x2_w, x2_t = y_list[2].size(3), y_list[2].size(4), y_list[2].size(2)
        x2 = torch.cat([y_list[2], \
                        F.interpolate(y_list[1], size=(x2_t, x2_h, x2_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x2 = self.se_block1(x2)
        x2 = self.basic_block6(x2)
        outs.append(x2)
        # x0 = self.conv0(x0)

        # 第二个主流
        x1_h, x1_w, x1_t = y_list[1].size(3), y_list[1].size(4), y_list[1].size(2)
        x1 = torch.cat([y_list[1], \
                        F.interpolate(y_list[0], size=(x1_t, x1_h, x1_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x1 = self.se_block2(x1)
        x1 = self.basic_block3(x1)
        x3 = torch.cat([y_list[2], \
                        F.interpolate(x1, size=(x2_t, x2_h, x2_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x3 = self.se_block1(x3)
        x3 = self.basic_block6(x3)
        outs.append(x3)

        # 两个主流进行融合与分权重
        x0_h, x0_w, x0_t = outs[1].size(3), outs[1].size(4), outs[1].size(2)
        x0 = torch.cat([outs[1], \
                        F.interpolate(outs[0], size=(x0_t, x0_h, x0_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x0 = self.se_block3(x0)
        x0 = self.basic_block7(x0)

        # 融合的与主流进行融合
        x4 = torch.cat([y_list[2], \
                        F.interpolate(x0, size=(x2_t, x2_h, x2_w), mode='trilinear', align_corners=True), \
                        ], 1)
        x4 = self.se_block3(x4)
        x4 = self.basic_block7(x4)
...
# input = torch.randn(1, 17, 32, 56, 56)
# model = ResNet3d()
# model.forward(input)
